...
if (titleTextContent == "Streamtape.com" || document.querySelector('meta[name="og:sitename"][content="Streamtape.com"]')) {
    let clickInitialize = setInterval(function clickInit() {
        document.querySelector("[data-name='stitialer']+div")?.click();
        const element = document.querySelector(".plyr-overlay") || document.querySelector(".play-overlay");
        if (element) {
            element.click();
            clearInterval(clickInitialize);
...
else if (document.querySelector("meta[name='keywords']")?.content?.includes("VOE")) { //VOE
...
//# sourceMappingURL=extractVideo.js.map