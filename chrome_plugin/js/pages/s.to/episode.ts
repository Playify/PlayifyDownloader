		...
		for(const node of mutation.addedNodes as any as HTMLIFrameElement[])
			if(node.tagName==="IFRAME"&&node.hasAttribute("allowfullscreen")){
				...
				new MutationObserver((iframeMutations,observer)=>{
					...
					if(!blockIframes) return observer.disconnect();
					if(!loadBestHost(node)) return;//need to look at it again
					observer.disconnect();
...
function loadBestHost(iframe:HTMLIFrameElement){
	...
	if(isNew){
		...
		const hosts:[string,Language][]=[
			["VOE",language],
			["Vidoza",language],
		]

		for(let [host,lang] of hosts){
			const element=document.querySelector<HTMLButtonElement>(`button:has(img[alt='${host}']):has(use[href='#icon-flag-${lang}'])`);
			if(element){
		...
		const elementFallback=document.querySelector<HTMLButtonElement>(`button:has(use[href='#icon-flag-${language}'])`);
		if(elementFallback){
